...
**Pump.fun markets** are binary prediction questions tied to a specific pump.fun coin mint. Each market asks whether something will happen to that coin — graduate from the bonding curve, cross a market-cap target, or finish a five-minute window up or down.

Unlike the raw pump.fun coin feed, ezpz.fi only lists **platform-created markets**. A coin appears on `/pumpfun` only after a maker or the platform publishes a market for it. You never browse coins that have no tradeable question on ezpz.

<Note>
  Pump.fun markets cannot be used as parlay legs yet. Parlays are soccer-only today. Use single predictions on pump.fun markets. See [Parlays](/concepts/parlays).
</Note>
...
| **Market-cap threshold** | Will MOON cross $5m mcap by Friday? | Recorded USD market cap at expiry |
...
|--------|-----|-------------------|
| **Maker / community author** | Any signed-in maker (or player who switches to maker) | Graduation, mcap-threshold, or 5-minute up/down markets on a chosen coin |
| **Platform factory** | Automated cron on trending coins | Mcap-threshold markets only — one per coin while no open market exists |

Community members do not need to be the coin's pump.fun creator to publish. Anyone with a maker account can author a market on any coin mint. Optional **verified creator** status is a badge, not a gate — see [For pump.fun creators & community](/guides/pumpfun-creators).
...
| USD market cap | Snapshot recorder (60s cadence) | Mcap-threshold markets |
| Price (mcap ÷ supply) | pump.fun lookup at window open and close | Up/down markets |
...
There is **no upfront authoring fee** for pump.fun markets. The maker's **AMM seed liquidity** is the commitment — USDC collateral that backs the binary payout, the same as other maker markets.
