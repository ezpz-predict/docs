...
| **Solana Ed25519 precompile** | CLOB `OrderIntent` signature verification in `authoring-exchange` |
...
| --- | --- | --- |
| **AMM trade prices** | `authoring-amm` pool state (`q_yes`, `q_no`, LS-LMSR) | Updates only when someone trades (swap / mint path) |
