...
ezpz.fi prices AMM trades with **LS-LMSR** (_liquidity-sensitive Logarithmic Market Scoring Rule)_ (Othman, Sandholm, Pennock, Reeves, EC 2010). It is a **cost-function market maker**: the pool tracks how many outcome tokens it has sold and charges USDC equal to the change in a convex cost function $$C(q)$$.
...
That is why seed USDC is the maker's collateral against subsidy**.** Which is vital because unlike token creators that do no renounce their LP, this prediction market pool cannot lose more than the seed in the adversarial orthant bound.
