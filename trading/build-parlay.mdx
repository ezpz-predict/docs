...
description: "Combine multiple soccer legs into one accumulator bet."
icon: "layer-group"
---

## Overview

A parlay lets you combine multiple selections into one bet. All legs must win for a payout. Odds multiply, so potential returns are higher — but risk increases with each leg added.
...
## Surrender on parlays

Surrender (cash-out) is only available when **every leg** is more than 20 minutes from its event end. If any single leg is too close to finishing, surrender is blocked for the entire parlay.
