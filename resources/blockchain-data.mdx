...
| Market state (open/resolved/void) | Yes | `Market` account on `ezpz-market` |
| AMM pool reserves | Yes | `ezpz-amm` pool account |
| Parlay stake and legs | Yes | `Parlay` account on `ezpz-parlay` |
...
<Steps>
  ...
  <Step title="Inspect program calls">
    Expand instructions to see calls to `ezpz-market`, `ezpz-amm`, or `ezpz-parlay`.
  </Step>
</Steps>
...
The platform runs **ezpz-indexer**, which mirrors on-chain events into Postgres for the UI (Portfolio, charts, activity feed). The indexer is a **projection**, Solana is the source of truth for balances and settlement.
